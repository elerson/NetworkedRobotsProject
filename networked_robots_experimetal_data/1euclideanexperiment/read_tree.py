...
class TreeSegmention:
    ...
    def doAllocation(self, ray):
        ...
        connection = path[0]
        intesection_points = np.matrix([tree.graph_vertex_position[connection[0]]])
 
        for connection in path:
            
            #get the closest point from the allocated position to the new allocation (within a distance ray)
            index_ = self.getClosestPoints(intesection_points, tree.graph_vertex_position[connection[0]], ray)            
            ...
            for i in index:
                ...
                p, dist = self.getFirstPoint(closest_point, tree.graph_vertex_position[connection[0]], tree.graph_vertex_position[connection[1]], ray)
                if(dist < min_dist):
                    min_dist = dist
                    allocation_point = p

            #add point for the first allocation
            if(len(resulting_allocation) == 1):
            ...
            allocation = self.segmentTreeFromPoint(allocation_point, tree.graph_vertex_position[connection[1]], ray)
            intesection_points = np.append(intesection_points, allocation[-1], 0)
...
class Robot:
    def __init__(self, id):
        self.id = id
        self.position = []
        self.all_robots_positions = {}

    def getDistance(self, p1, p2):
        return math.sqrt((p1[0]-p2[0])**2 + (p1[1]-p2[1])**2)


    def doAllocation(self, tree_allocation_positions):
        n_robots = len(self.all_robots_positions)
        n_allocations = len(tree_allocation_positions)
        cost_matrix = np.zeros((n_robots, n_allocations))

        for r in range(n_robots):
            for t in range(n_allocations):
                cost_matrix[r, t] = self.getDistance(self.all_robots_positions[r], tree_allocation_positions[t])


        row_ind, col_ind = linear_sum_assignment(cost_matrix)
        if( self.id in row_ind ):
            index = col_ind.tolist().index(self.id)
...
if __name__ == "__main__":
    ...
    ray = 43.99
    leafs = tree. graph_vertex_position
    allocation = segmentation.doAllocation(ray)
    print(len(allocation))

    # for i in range(4,7):
    #     print(i)
    #     for j in range(len(allocation)):
    #         point = (allocation[j,0], allocation[j,1])
    #         dist = segmentation.getDistance(leafs[i], point)
    #         print(dist)
