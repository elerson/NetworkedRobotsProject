...
<launch>
<group ns = "robot_ID">
  <node pkg="robots_deployment_offline" type="deployment.py" name="ros_deployment_ID" output="screen">
...
class Create():
  ...
  def create(self, numfiles):
    ...
    with open("all.launch", 'w') as f:
      text = "<launch>\n"
      for x in range(1, numfiles):
        text += "<include file=\"$(find robots_deployment_offline)/launch/scripts/"+files[x]+ "\" />\n"
      text += "</launch>\n"
      f.write(text)
