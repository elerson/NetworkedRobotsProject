    ...
    with open("all_robots.launch", 'w') as f:
      ...
      for x in range(1, numfiles):
        with open(files[x], 'w') as f:
          f.write(robot_exp.replace('ID', str(x-1)).replace('RADIUS', str(radius)).replace('X', str(self.x)).replace('Y', str(self.y)))
...
class CreateTerminal():
  ...
  def create(self, numfiles, radius):
    ...
    with open("all.launch", 'w') as f:
      ...
      for x in range(1, numfiles):
        with open(files[x], 'w') as f:
          f.write(terminals_exp.replace('ID', str(x-1)).replace('RADIUS', str(radius)).replace('X', str(self.x)).replace('Y', str(self.y)))
...
if __name__ == "__main__":
  ...
  terminals.create(5, radius*resolution)

  robots         = CreateRobots()
  robots.create(num_robots, radius*resolution)
