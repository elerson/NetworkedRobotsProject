...
class Image(QtGui.QWidget):
    ...
    def paintTree(self, painter):
        ...
        for i in self.graph:
            for j in self.graph[i]:
                ...
                pen.setWidth(5)
                painter.setPen(pen)
                painter.drawPoint(self.offset +  self.offset_ + initial_pose)
                painter.drawPoint(self.offset +  self.offset_ + end_pose)
    ...
    def plotImage(self, painter):
        ...
        pen.setWidth(10)
        painter.setPen(pen)
        for robot_id in self.terminals:
            position = QPointF(self.terminals[robot_id][0], self.terminals[robot_id][1])
            painter.drawPoint(self.offset +  self.offset_ + position)
