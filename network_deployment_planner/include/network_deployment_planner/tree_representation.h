...
private:
	...
	bool recursiveSearch(int source_node_index, int calledby, int first_destination_node_index, int second_destination_node_index, std::vector<int> &path);
...
public:
	...
	std::map<int, point2d_>& getVertices();
	bool isClient(int i){return clients_[i];}
	int numClient(){
		int numclients = 0;
		for(int i = 0; i < num_points_; i++)
			if(clients_[i]) numclients++;
		return numclients;
	}
